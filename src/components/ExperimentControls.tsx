...
export function ExperimentControls({ language, experiment, config, world, prediction, onPredict, onReset, send }: Props) {
  ...
  const [noise, setNoise] = useState(world.config.behavior.danceNoise), [seedError, setSeedError] = useState(false);
  useEffect(() => { setSeed(String(config.seed)); setScouts(config.behavior.scoutRatio); setSeedError(false); }, [config]);
  ...
  return <aside className="experiment-rail panel" aria-label={t('Experiment controls', 'Deney kontrolleri')}>
    ...
    <section className="controls-section"><h3>{t('Colony controls', 'Koloni kontrolleri')}</h3>
      ...
      <div className="range-field"><div className="control-caption"><label htmlFor="scouts">{t('Scout proportion', 'Keşifçi oranı')}</label><TermHelp term="scouts" language={language} /><output htmlFor="scouts">{Math.round(scouts * 100)}%</output></div>
        <input id="scouts" type="range" min="0.01" max="1" step="0.01" value={scouts} onChange={e => setScouts(Number(e.target.value))} onPointerUp={applyScouts} onKeyUp={applyScouts} onBlur={applyScouts} />
      ...
      <form className="seed-field" onSubmit={e => { e.preventDefault(); const value = Number(seed);
        if (!/^\d+$/.test(seed) || !Number.isSafeInteger(value) || value < 0 || value > 4294967295) { setSeedError(true); return; }
        setSeedError(false); onReset({ ...config, seed: value }); }}>
        <div className="control-caption"><label htmlFor="seed">Seed</label><TermHelp term="seed" language={language} /></div><div><input id="seed" inputMode="numeric" value={seed} onChange={e => setSeed(e.target.value)} aria-invalid={seedError} aria-describedby={seedError ? "seed-help seed-error" : "seed-help"} /><button type="submit">{t('Apply', 'Uygula')}</button></div>
        {seedError && <p id="seed-error" className="error" role="alert">{t('Use an integer from 0 to 4294967295.', '0–4294967295 arasında bir tam sayı girin.')}</p>}
      </form>
