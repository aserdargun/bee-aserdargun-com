...
export function ExperimentControls({ language, experiment, config, world, prediction, onPredict, onReset, send }: Props) {
  ...
  return <aside className="experiment-rail panel" aria-label={t('Experiment controls', 'Deney kontrolleri')}>
    ...
    <section className="controls-section"><h3>{t('Colony controls', 'Koloni kontrolleri')}</h3>
      <label className="toggle-row"><span>{t('Dance recruitment', 'Dansla katılım')}</span><span className="switch"><input aria-label={t('Dance recruitment', 'Dansla katılım')} type="checkbox" checked={world.config.behavior.recruitment} onChange={e => send({ type: 'intervene', command: { type: 'behavior', recruitment: e.target.checked } })} /><span className="switch-track" /></span></label>
      <div className="range-field"><label htmlFor="scouts">{t('Scout proportion', 'Keşifçi oranı')}<output>{Math.round(scouts * 100)}%</output></label>
      ...
      <div className="range-field"><label htmlFor="noise">{t('Communication noise', 'İletişim gürültüsü')}<output>{Math.round(noise * 100)}%</output></label>
      ...
      <form className="seed-field" onSubmit={e => { e.preventDefault(); const value = Number(seed);
        ...
        <label htmlFor="seed">Seed</label><div><input id="seed" inputMode="numeric" value={seed} onChange={e => setSeed(e.target.value)} aria-describedby="seed-help" /><button type="submit">{t('Apply', 'Uygula')}</button></div>
