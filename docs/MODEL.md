...
The September 10 hardening keeps schema 1 and all model versions at 0.1.0: no transition, random draw, world constant, metric formula or experiment initial condition changed. Existing valid standard-policy runs retain their interpretation. Validation now enforces the documented supported-field and portable-export boundaries. The internal Worker import acknowledgement includes the accepted initial configuration; the UI blocks competing actions until replay completes.
