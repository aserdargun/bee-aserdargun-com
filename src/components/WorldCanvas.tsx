...
export function WorldCanvas(props: Props) {
  ...
  useEffect(() => { optionsRef.current = props; dirty.current = true; }, [props]);
  ...
  useEffect(() => {
    const img = new Image(); img.src = '/art/meadows-paper.png'; img.onload = () => { artRef.current = img; dirty.current = true; };
    ...
    const resize = new ResizeObserver(([entry]) => {
      dirty.current = true; width = entry.contentRect.width; height = entry.contentRect.height;
      const dpr = Math.min(window.devicePixelRatio || 1, 2);
      canvas.width = Math.round(width * dpr); canvas.height = Math.round(height * dpr); ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
    }); resize.observe(canvas);
    const render = (now: number) => {
      const p = optionsRef.current;
      const blending = p.playing && now - data.current.time < 100;
      if (width && height && (dirty.current || blending)) {
        dirty.current = blending;
        count++;
        drawWorld(ctx, width, height, data.current.current, data.current.previous,
      ...
      if (now - last > 1500) { if (p.debug) setFps(Math.round(count * 1000 / (now - last))); count = 0; last = now; }
  ...
  const changeZoom = (next: number) => { dirty.current = true; cameraRef.current.zoom = Math.min(3, Math.max(0.75, next)); setZoom(cameraRef.current.zoom); };
  ...
  return <div className={`world-canvas ${props.compact ? 'compact' : ''}`}>
    <canvas ref={canvasRef} aria-label={props.language === 'tr' ? 'Canlı koloni. Arı seçimi için Arıyı incele kontrolünü de kullanabilirsiniz.' : 'Live colony. Use Inspect a bee for keyboard-accessible selection.'}
      ...
      onPointerMove={e => { const d = drag.current; if (!d) return; const dx = e.clientX - d.x, dy = e.clientY - d.y;
        if (Math.abs(dx) + Math.abs(dy) > 3) { d.moved = true; if (cameraRef.current.zoom > 1) {
          ...
          dirty.current = true; cameraRef.current.x -= dx / tr.scale; cameraRef.current.y -= dy / tr.scale;
    ...
    <div className="camera-tools">
      ...
      <button aria-label={props.language === 'tr' ? 'Kamerayı sıfırla' : 'Reset camera'} onClick={() => { dirty.current = true; cameraRef.current = { zoom: 1, x: 0, y: 0 }; setZoom(1); }}><Crosshair size={17} /></button>
    </div>
